...
| 服务 | 文件 | 功能 |
|------|------|------|
| `InputActivityState` | `scripts/services/InputActivityState.cs` | 输入数据聚合、AP 计算、滑动窗口硬封顶 |
...
**数值公式**: `AP = key×1.0 + click×1.2 + scroll×0.4 + move/600`，滑动窗口硬封顶 `MaxInputPerMinute = 600`（超出部分丢弃）
...
│  │ • 滑动窗口硬封顶：MaxInputPerMinute = 600（超出丢弃） │ │
│  │ • 累计统计（用于存档）                               │   │
│  └────────────────────────┬────────────────────────────┘   │
│                           │                                 │
│                           ▼                                 │
...
- 按 1 秒时间片聚合原始输入
- 不记录具体键值，仅统计次数
- 滑动窗口硬封顶：每分钟最大有效输入 600 次，超出直接丢弃
- 累计总统计值用于持久化
...
统计口径 = key_down + mouse_click + scroll_step（鼠标移动不计入）
60 秒滑动窗口内超过 MaxInputPerMinute 的输入事件直接丢弃，不计入 AP。
不给任何 UI 提示（避免暴露检测逻辑）。
```

> 原"高频衰减"公式（`decay = clamp(1 - max(0, R-1)×0.25, 0.45, 1)`）已移除，
> 由硬封顶替代。硬封顶比柔性衰减更简单直接：正常用户永远不会触及 600/min
> （约 10 次/秒，超过人类正常打字极限），脚本刷分则被完全截断。
...
GD.Print($"AP applied: {activityState.ApFinal}");
...
{
    // 使用 apFinal 计算游戏进度
    // 当前硬封顶模型下，apThisSecond 与 apFinal 一致
}
```
...
| InputWindowSeconds | 60.0 | 硬封顶统计窗口长度 |
| MaxInputPerMinute | 600 | 60秒滑动窗口内最大有效输入次数 |
...
### Q: 如何防止脚本刷分？
A: 滑动窗口硬封顶机制会丢弃 60 秒内超过 600 次的输入，超出部分不计入 AP。同时 `ApAccumulator` 缓冲区会平滑突发输入，避免瞬间大量操作直接转化为游戏进度。
...
- [ ] 高频输入时（快速按键），60 秒内超过 600 次的离散输入不再继续提升 AP/进度
