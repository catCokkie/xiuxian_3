...
public sealed class ExploreProgressPresentationRulesTests
{
    ...
    {
        Assert.Equal("副本｜材料装备", ExploreProgressPresentationRules.GetActionModeOptionText(PlayerActionState.ModeDungeon));
        Assert.Equal("灵田｜草花果", ExploreProgressPresentationRules.GetActionModeOptionText(PlayerActionState.ModeGarden));
        Assert.Equal("灵渔｜鱼珠涎", ExploreProgressPresentationRules.GetActionModeOptionText(PlayerActionState.ModeFishing));
        Assert.Equal("体修｜永久属性", ExploreProgressPresentationRules.GetActionModeOptionText(PlayerActionState.ModeBodyCultivation));
        Assert.Equal("炼丹｜战斗丹药", ExploreProgressPresentationRules.GetActionModeOptionText(PlayerActionState.ModeAlchemy));
        Assert.Equal("副本｜材料装备", ExploreProgressPresentationRules.GetActionModeOptionText(0));
        Assert.Equal("体修｜永久属性", ExploreProgressPresentationRules.GetActionModeOptionText(10));
    ...
    public void ProgressTexts_FormatCraftingAndGatheringStatus()
    {
        ...
        Assert.Equal("种植：灵花 已成熟，可收获", ExploreProgressPresentationRules.BuildGardenProgressText("灵花", 240, 240));
    ...
    public void BuildRecentBattleLogText_FormatsEntries()
    {
        string text = ExploreProgressPresentationRules.BuildRecentBattleLogText(
            new[] { ("12:34", "干泉洞窟", "苔痕史莱姆", "normal", 3, "胜利", "灵气+12") });
    ...
    {
        string text = ExploreProgressPresentationRules.BuildRecentBattleLogText(
            new[] { ("12:34", "干泉洞窟", "暗窟蛛", "elite", 5, "胜利", "灵草x2") });
    ...
    {
        string text = ExploreProgressPresentationRules.BuildRecentBattleLogText(
            new[] { ("12:34", "干泉洞窟", "阴潮蛇", "normal", 2, "失败", "none") });
